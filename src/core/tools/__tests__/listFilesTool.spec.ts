...
describe("ListFilesTool", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(resolvePathInWorkspace).mockImplementation(async (cwd, relPath) => path.resolve(cwd, relPath))
		vi.mocked(getWorkspaceReadablePath).mockImplementation(
			(_cwd, _absolutePath, fallbackPath) => fallbackPath ?? "src",
		)
	...
	it("should list files in a directory (non-recursive)", async () => {
		...
		expect(resolvePathInWorkspace).toHaveBeenCalledWith(mockTask.cwd, "src")
	...
	it("should resolve relative path against task.cwd", async () => {
		...
		expect(resolvePathInWorkspace).toHaveBeenCalledWith(mockTask.cwd, "relative/path")
	...
	it("should use getWorkspaceReadablePath for the message path", async () => {
		const absolutePath = path.resolve(TEST_WORKSPACE, "src/utils")
		vi.mocked(resolvePathInWorkspace).mockResolvedValue(absolutePath)
		vi.mocked(getWorkspaceReadablePath).mockReturnValue("secondary/src/utils")
		const params = { path: "src/utils", recursive: false }

		await tool.execute(params, mockTask, mockCallbacks)

		expect(getWorkspaceReadablePath).toHaveBeenCalledWith(TEST_WORKSPACE, absolutePath, "src/utils")
		expect(mockCallbacks.askApproval).toHaveBeenCalledWith(
			"tool",
			expect.stringContaining('"path":"secondary/src/utils"'),
		)
	})

	it("should list files from the resolved workspace path and surface a secondary-root readable path", async () => {
		const relPath = "secondary/src"
		const absolutePath = "/workspace/secondary/src"
		mockTask.rooIgnoreController = {} as any
		mockTask.rooProtectedController = {} as any
		vi.mocked(mockTask.providerRef.deref).mockReturnValue({
			getState: vi.fn().mockResolvedValue({ showRooIgnoredFiles: true }),
		} as any)
		vi.mocked(resolvePathInWorkspace).mockResolvedValue(absolutePath)
		vi.mocked(getWorkspaceReadablePath).mockReturnValue("secondary/src")
		vi.mocked(listFiles).mockResolvedValue([["a.ts", "b.ts"], false])

		await tool.execute({ path: relPath, recursive: true }, mockTask, mockCallbacks)

		expect(resolvePathInWorkspace).toHaveBeenCalledWith(TEST_WORKSPACE, relPath)
		expect(listFiles).toHaveBeenCalledWith(absolutePath, true, 200)
		expect(formatResponse.formatFilesList).toHaveBeenCalledWith(
			absolutePath,
			["a.ts", "b.ts"],
			false,
		...
		expect(mockCallbacks.askApproval).toHaveBeenCalledWith(
			"tool",
			expect.stringContaining('"path":"secondary/src"'),
		)
	})
	...
	it("should construct correct shared message props for non-recursive listing", async () => {
		vi.mocked(getWorkspaceReadablePath).mockReturnValue("src")
		const params = { path: "src", recursive: false }
	...
	it("should handle partial message with no path (defaults to task.cwd)", async () => {
		vi.mocked(getWorkspaceReadablePath).mockReturnValue("")
		const block = createBlock({}, true)

		await tool.handlePartial(mockTask, block)

		expect(resolvePathInWorkspace).not.toHaveBeenCalled()
		expect(getWorkspaceReadablePath).toHaveBeenCalledWith(TEST_WORKSPACE, TEST_WORKSPACE, "")
	...
	it("should resolve path relative to task.cwd in handlePartial", async () => {
		const absolutePath = path.resolve(TEST_WORKSPACE, "nested/dir")
		vi.mocked(resolvePathInWorkspace).mockResolvedValue(absolutePath)
		vi.mocked(getWorkspaceReadablePath).mockReturnValue("nested/dir")
		const block = createBlock({ path: "nested/dir" }, true)

		await tool.handlePartial(mockTask, block)

		expect(resolvePathInWorkspace).toHaveBeenCalledWith(TEST_WORKSPACE, "nested/dir")
		expect(getWorkspaceReadablePath).toHaveBeenCalledWith(TEST_WORKSPACE, absolutePath, "nested/dir")
	})

	it("should use the resolved workspace path in partial payloads for a secondary root", async () => {
		const relPath = "secondary/src"
		const absolutePath = "/workspace/secondary/src"
		vi.mocked(resolvePathInWorkspace).mockResolvedValue(absolutePath)
		vi.mocked(getWorkspaceReadablePath).mockReturnValue("secondary/src")
		const block = createBlock({ path: relPath, recursive: "true" as any }, true)

		await tool.handlePartial(mockTask, block)

		expect(resolvePathInWorkspace).toHaveBeenCalledWith(TEST_WORKSPACE, relPath)
		expect(getWorkspaceReadablePath).toHaveBeenCalledWith(TEST_WORKSPACE, absolutePath, relPath)
		expect(mockTask.ask).toHaveBeenCalledWith("tool", expect.stringContaining('"path":"secondary/src"'), true)
		expect(mockTask.ask).toHaveBeenCalledWith("tool", expect.stringContaining('"tool":"listFilesRecursive"'), true)
	})
