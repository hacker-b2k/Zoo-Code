...
import { StatePushDebouncer } from "./StatePushDebouncer"
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	 ...
	 * within a single animation frame (16 ms) into a single push.
	 */
	private readonly statePushDebouncer: StatePushDebouncer = new StatePushDebouncer(
		{ postStateToWebview: () => this.doPostStateToWebview() },
		16,
	)
	...
	*/
	...
	/**
	 * Schedule a debounced state push. Multiple calls within 16 ms are
	 * coalesced into a single push.  Returns a promise that resolves after
	 * the push completes.
	 */
	...
	/**
	 * The actual (un-debounced) state push implementation.
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		return {
			...
			currentTaskIsBackgroundWorker: currentTask?.isBackgroundWorker ?? false,
			...
			imageGenerationProvider,
			imageGenerationBaseUrl,
			imageGenerationApiKey: imageGenerationApiKey || openRouterImageApiKey,
			imageGenerationHeaders,
			imageGenerationSelectedModel: imageGenerationSelectedModel || openRouterImageGenerationSelectedModel,
			imageGenerationApiMethod,
			imageGenerationCustomProvider,
			vertexImageProjectId,
			vertexImageRegion,
	...
	> {
		...
		const result = {
			...
			imageGenerationBaseUrl: stateValues.imageGenerationBaseUrl,
			imageGenerationApiKey: stateValues.imageGenerationApiKey || stateValues.openRouterImageApiKey,
			imageGenerationHeaders: stateValues.imageGenerationHeaders,
			imageGenerationSelectedModel:
				stateValues.imageGenerationSelectedModel || stateValues.openRouterImageGenerationSelectedModel,
			imageGenerationApiMethod: stateValues.imageGenerationApiMethod,
			imageGenerationCustomProvider: stateValues.imageGenerationCustomProvider,
			vertexImageProjectId: stateValues.vertexImageProjectId,
			vertexImageRegion: stateValues.vertexImageRegion,
			vertexImageModel: stateValues.vertexImageModel,
			vertexImageAuthMode: stateValues.vertexImageAuthMode,
			vertexImageAccessToken: stateValues.vertexImageAccessToken,
			vertexImageServiceAccountJson: stateValues.vertexImageServiceAccountJson,
