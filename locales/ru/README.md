...
- **Интерфейс управления правилами** — новая вкладка «Правила» в настройках позволяет создавать, удалять и открывать глобальные и рабочие правила Zoo прямо из редактора
- **Проверка изменений после завершения** — после завершения задачи новые кнопки «Посмотреть новые изменения» и «Восстановить изменения» позволяют проверить и отменить изменения из последнего запроса
- **Смягчённые пороги diff** — настраиваемые пороги сходства уменьшают ошибки «редактирование не удалось» при применении diff, а новые диагностические инструменты помогают настроить параметры
