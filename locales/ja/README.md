...
- **ルール管理UI** — 設定の新しい「ルール」タブで、グローバルおよびワークスペースのZooルールをエディターから直接作成、削除、開くことができます
- **完了変更レビュー** — タスクが完了すると、新しい「新しい変更を表示」「変更を復元」ボタンで最後のプロンプトの変更を確認・取り消しできます
- **緩和されたDiff閾値** — 設定可能な類似度閾値により、diff適用時の「編集失敗」エラーが減少し、設定の調整に役立つ新しい診断機能が追加されました
